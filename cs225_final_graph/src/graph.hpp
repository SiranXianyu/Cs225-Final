...
struct Node {
    ...
    std::string name;
    unsigned occurrence_times; // Jialong Li 
    double centrality;
};
...
class Graph {
    ...
    void ReadRelations() {
        ...
        file_stream.open("data_edge_test2.csv");
        std::string token;
    ...
    void ReadVertices() {
        std::ifstream file_stream;
        file_stream.open("data_vertex_test2.csv");
        std::string token;
    ...
    void ReadSchools() {
        ...
        for (unsigned i = 0; i < schools.size() - 1; i++) {
                Node college;
                college.school_number = i; // for checking
                college.name = schools.at(i).first;
                college.occurrence_times = schools.at(i).second; // Jialong Li
                nodes_.push_back(college);
        }
    }

    // read adjacency list
    // Basically, this is a map<school, all_neighbors>.
    void ReadEdges() {
        ...
        /*
        std::cout << "test ReadEdges" << std::endl;
    ...
    // stores only neighbor of a school, give a index to each neighbors
    // havard (0) -> neighbors
    // call 0 -> get all neighbors.
    void RefineLists() {
    ...
    std::vector<double> brandes(){
        ...
        for (unsigned i = 0; i < nodes_.size(); i++) {
            nodes_.at(i).centrality = centrality_[i];
        }
